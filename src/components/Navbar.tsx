...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-gray-900/95 backdrop-blur-lg border-b border-purple-900/20">
            ...
            <div className="flex items-center space-x-1 ml-2">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="px-3 py-2 text-sm font-medium text-gray-300 hover:text-purple-400 transition-colors duration-200"
                >
                  {t(link.label)}
                </a>
              ))}
      ...
      {/* Mobile menu */}
      {isOpen && (
        <div className="md:hidden bg-gray-900/95 backdrop-blur-lg border-b border-purple-900/20">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="block px-3 py-2 text-base font-medium text-gray-300 hover:text-purple-400 hover:bg-purple-900/20 rounded-lg transition-all duration-200"
                onClick={toggleMenu}
              >
                {t(link.label)}
              </a>
